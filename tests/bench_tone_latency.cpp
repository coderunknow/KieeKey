...
//============================================================================
//----------------------------------------------------------------------------
// KieeKey v1.0.1 — tests/bench_tone_latency.cpp
...
void runPopulation(TextEngine& eng, const PopDef& p, Path path, const char* pathName,
    ...
    for (std::size_t i = 0; i < nKeys; ++i) {
        ...
        if (pacedMs > 0.0) {
            std::this_thread::sleep_until(
                nextKey + std::chrono::duration<double, std::milli>(
                              pacedMs * static_cast<double>(i + 1)));
        }
    }
    ...
    std::printf("      EDIT  n=%-5zu E2E(t5) p50=%7.3f p99=%8.3f p99.9=%9.3f max=%9.3f us\n",
                ...
                eE2e.empty() ? 0.0
                             : static_cast<double>(*std::max_element(eE2e.begin(), eE2e.end())) / 1000.0);
    ...
    std::printf("      PASS  n=%-5zu delivery(t2) p50=%7.3f p99=%8.3f max=%9.3f us | "
                ...
                pT2.empty() ? 0.0
                            : static_cast<double>(*std::max_element(pT2.begin(), pT2.end())) / 1000.0,
                pct(pBar, 0.50),
                pBar.empty() ? 0.0
                             : static_cast<double>(*std::max_element(pBar.begin(), pBar.end())) / 1000.0);
...
int main(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        // Unique names per branch: the init-declaration of an `if` stays in
        // scope through the whole else-if chain, so reusing `v` shadows the
        // previous declaration (C4456 under MSVC /W4 /WX).
        if (const char* vPop = val(argv[i], "--pop=")) { popSel = vPop; }
        else if (const char* vPath = val(argv[i], "--path=")) { pathSel = vPath; }
        else if (const char* vBarrier = val(argv[i], "--barrier=")) { barrierSel = vBarrier; }
        else if (const char* vCommitNs = val(argv[i], "--commit-ns=")) { commitNsOpt = std::atol(vCommitNs); }
        else if (const char* vCommit = val(argv[i], "--commit=")) { commitSel = vCommit; }
    ...
    std::printf("[tone] KieeKey v1.0.1 — tone-population latency benchmark\n");
