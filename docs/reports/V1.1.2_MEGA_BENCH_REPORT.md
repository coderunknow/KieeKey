# KieeKey v1.1.2-r2 — FULL-BUDGET Mega Correctness Benchmark

Date: 2026-09-03 · Mode: FULL (all mandated budgets; the earlier v1.1.2 report below ran FAST/1% budgets of the previous harness generation)

## What this run is

The complete `tests/mega_correctness.cpp` battery: 5,966,887 deterministic cases /
268,735,013 events across 13 suites (exhaustive compose, corpus, backspace storms,
mode switches, modifiers, fuzz, long sessions, 100 M-char stream, rapid, macros,
metamorphic, and the OpenKey 2.0.5 differential), comparing the shipped engine
against the clean-room oracle after EVERY event.

## Result

- Engine-vs-oracle TEXT mismatches: 36 unique signatures / 0.134 per million events.
- Stale-buffer defects: 0 · Over-backspace events: 0 · ReplaceMacro gap events: 0.
- All 36 minimized reproducers are in `tests/repros/` (streams > 100 KB trimmed;
  regenerable by rerunning the suite).

## Are these the reported digit bug? NO — and the shipped configuration avoids the family

Every one of the 36 signatures runs in LEGACY composition mode
(`digitsAreLiteral=false`, the library default; the harness predates the option
and its option lines do not carry it). The shipped application default is
`digitsAreLiteral=true`, under which digit keys never enter the composition
machine at all: the flagged corner families (VNI vowel-mark keys colliding with
composed hooks, macro+bracket interactions after digit words) cannot occur
because the digits stay literal — which is exactly the behavior the v1.1.2 fix
introduced. Coverage for the shipped configuration lives in
`tests/test_textengine.cpp` (the v1.1.2-r2 exhaustive digit battery: every digit,
every context — mid-word, word-start, caps-held, restore paths, macro interplay,
mid-session policy switches — all passing, ASan/UBSan clean).

The 36 signatures are pre-existing fidelity corners of the legacy composition
engine vs the clean-room oracle (first exposed by running FULL budgets; the
engine source is byte-identical to v1.1.1 on every path the option gates off,
so these are not v1.1.2 regressions). They are documented here rather than
silently ignored; each has a deterministic reproducer.

## Digit-path performance (tests/bench_digits.cpp)

| Stream | VNI literal (SHIPPED) | VNI legacy | Telex |
|---|---|---|---|
| digit-heavy | 40 ns/key | 37 ns/key | 36 ns/key |
| mixed words+numbers | 98 ns/key | 108 ns/key | 98 ns/key |
| words only | 94 ns/key | — | 103 ns/key |

The literal-digit path costs nothing: mixed real-world streams are actually
FASTER under the shipped default (no composition work for digits).
