# KieeKey v1.3.0 RC1–RC2 — engine-decision benchmark and optimisation record
...
Two scope notes on what this page renders. The spliced campaign is the **strict** tree's release campaign of
record (`rc1-v13rel`), and it is the campaign whose numbers describe what ships enabled by default.
§8.2's two opt-in-profile campaigns (`rc1-v13prof` from RC1, `rc1-v13prof2` from RC2) are quoted as written
figures rather than spliced, because they are different artifact directories with their own bands and
order-controls; each is cited by name so nothing from them is read as a substitute for the strict column,
and neither is used to restate §1's headline.
...
### 8.2 The opt-in low-latency profile, and what a second lever bought
...
The ablation is the interesting part, because it prices each lever separately against the same UniKey
column and the same frozen v1.2.2 column (`rc1-v13prof` and `rc1-v13prof2`, medians over the same 5x12
sessions, ns/key):

| L1 cell | frozen v1.2.2 | UniKey | profile-A | profile-B | B vs UniKey |
|---|---:|---:|---:|---:|---:|
| `as-shipped` telex-end prose — deciding cell | 72.91 | 61.51 | 57.77 | **55.10** | **−10.41 %** |
| `as-shipped` telex-mid prose | 71.78 | 62.28 | 51.85 | 50.61 | −18.74 % |
| `as-shipped` vni prose | 66.28 | 59.05 | 52.87 | 50.53 | −14.43 % |
| `as-shipped` telex-end edit-storm | 83.45 | 65.09 | 59.50 | 55.85 | −14.20 % |
| `matched-minimal` telex-end prose | 53.81 | 41.59 | 37.70 | 36.01 | −13.42 % |
| `as-shipped` telex-end pathological | 32.30 | 27.77 | 31.89 | 30.91 | +11.33 % |
| `matched-minimal` vni pathological | 21.13 | 22.11 | 19.93 | 21.09 | −4.62 % |

`rc1-v13prof2`'s own paired statistic on the deciding cell reads **−7.69 % against UniKey** (60 paired
samples, A/A band 1.085 ns) with **+23.79 % over frozen** (CI [+16.88, +17.81] ns, ACCEPT, no cell
regressing beyond the band) — the first campaign in this programme whose pre-registered cell is negative.
The verdict is still **TIER MIXED**: three `pathological` cells regress against UniKey, which is the
adversarial stream the campaign builds on purpose, and `matched-minimal vni pathological` is the one cell
where the second lever costs rather than saves (+5.8 % slower than profile-A on an already-lean cell).

**What the two levers cost, and it is not equal.** Profile-A moves 52.03 % of the corpus's 715 474 keys to
a different repaint payload while leaving final text intact on 8 of 18 streams; profile-B moves **55.03 %**
(393 388 keys) and adds a second, narrower class of divergence — with the undo ring never written, a
backspace directly after a transform deletes a character instead of restoring the word's previous form.
Final-text-diverging streams stay at 10 of 18, so the second lever's price is paid mostly in payload bytes
and in one interaction, not in composed text. The `correctness` step's `digest-identity` gate fails on 22
of 376 rows for exactly this reason, and the row that makes it concrete is `soos ddieejn thoaji cura tooi`,
composing `Số điện thọai của tôi` where strict composes `Số điện thoại của tôi`. That gate failure is
published as the price, not smoothed over: a campaign of a text-changing configuration cannot satisfy either
`rc1_gates.py` policy (identity asserts row digests, declared-divergence asserts final text), so the step is
skipped by the runner and the divergence is measured by hand — the procedure is now in PROTOCOL §14.

Two things did *not* cost anything. The allocation-free `O(1)` core is intact under the profile: memory
gate PASS at **21 allocations per 2 000 000 keys** as-shipped (20 matched-minimal), RSS 46.2 MiB, identical
to the strict build's figures. And the strict build is untouched — its engine object is byte-identical to
`v1.3.0-rc1`'s, verified by compiling both trees and `cmp`-ing the objects, which is what keeps §8's
release numbers (the +15.74 % behind UniKey on the default) the description of this tree as well. That
proof is also why `undoHistory` became compile-time-only: the first implementation read an `EngineOptions`
...
467.5 vs 588.0, telex-end prose 368.0 vs 420.5. The two streams where `p50` reads behind (+3.5 and +4.0 ns,
both `telex-mid`) move less than the 5.10 ns column-order shift measured on this host in the same campaign,
so they are reported as undecidable; the `p99` gaps of +31.0 (vni prose) and +19.5 (telex-mid pathological)
are outside that and are regressions, published as such.
