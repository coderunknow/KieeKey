...
static void testBarrierSpinCatch() {
    ...
    // consumer "applies" the edit almost instantly
    std::thread consumer([&pending, &b] {
    ...
    const bool ok = b.waitDrained(pending);
    consumer.join();
    CHECK(ok);                            // drained within budget
    CHECK(pending.load() == 0);
}
...
static void testBarrierNotifyBeforeWait() {
    ...
    EditDrainBarrier b;
    std::atomic<std::uint32_t> pending{1};
    // consumer signals BEFORE the waiter enters the event wait — the
    // stateful auto-reset semantics must carry the signal
    std::thread consumer([&pending, &b] {
        std::this_thread::sleep_for(std::chrono::microseconds(500));
        pending.store(0, std::memory_order_release);
        b.notifyDrained();
    });
    const bool ok = b.waitDrained(pending, 2);   // 2 pause iters → event wait
    consumer.join();
    CHECK(ok);
}
...
static void testBarrierBudgetBounded() {
    ...
    CHECK(ms >= 0 && ms <= 60);              // ~1 ms budget + scheduler slack
