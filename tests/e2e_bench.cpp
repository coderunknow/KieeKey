...
struct BenchPipeline {
    ...
    void consumeLoop() {
        ...
        const auto pause = [] { __builtin_ia32_pause(); };
#else
        const auto pause = [] { std::this_thread::yield(); };
#endif
...
int main(int argc, char** argv) {
    ...
    const std::uint64_t spikes = std::count_if(sorted.begin(), sorted.end(),
    ...
    std::printf("[e2e] RAW      p50=%.3f us  p99=%.3f us  p99.9=%.3f us  max=%.3f us\n",
                ...
                sorted.empty() ? 0.0 : sorted.back() / 1000.0);
    ...
    std::printf("[e2e]          p50=%.3f us  p99=%.3f us  p99.9=%.3f us  max=%.3f us\n",
                ...
                hotOnly.empty() ? 0.0 : hotOnly.back() / 1000.0);
    ...
    std::printf("[e2e]          p50=%.3f us  p99=%.3f us  p99.9=%.3f us  max=%.3f us\n",
                ...
                pipelineOnly.empty() ? 0.0 : pipelineOnly.back() / 1000.0);
