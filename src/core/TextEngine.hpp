...
// Portable forced-inline for composeCached below. PR15 wrote this as a bare
// GNU __attribute__((always_inline)), which MSVC rejects (this header builds
// under /W4 /WX on x64/ARM64/ARM64EC) — the macro keeps the forced inline on
// every toolchain: __forceinline on MSVC, the GNU attribute on GCC/Clang,
// plain inline anywhere else.
#if defined(_MSC_VER)
#define KIEEKEY_ALWAYS_INLINE __forceinline
#elif defined(__GNUC__) || defined(__clang__)
#define KIEEKEY_ALWAYS_INLINE __attribute__((always_inline)) inline
#else
#define KIEEKEY_ALWAYS_INLINE inline
#endif
...
private:
    ...
    KIEEKEY_ALWAYS_INLINE std::uint32_t composeCached(std::size_t pos) noexcept {
