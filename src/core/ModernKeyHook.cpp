...
//============================================================================
//----------------------------------------------------------------------------
// KieeKey v1.1.0 — ModernKeyHook.cpp
...
// Runs on the HOOK PUMP thread only (the app calls it from the
// ForegroundChanged branch of its producer handler, which is serialized with
// the LL callbacks and applyModifierDelta on that same thread — no races on
// modifierBits_).
//
...
// the "tone marks make my letters uppercase" report. The Ctrl+Shift chord
// tracker got exactly this resync in v1.1.0; the modifier tracker — which
// feeds layoutChar()/produceChar() and therefore the ENGINE's case model —
