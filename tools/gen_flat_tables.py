...
def main():
    ...
    out = []
    out.append("//----------------------------------------------------------------------------")
    out.append("// KieeKey v1.0 — FlatTables.hpp (GENERATED by tools/gen_flat_tables.py)")
    out.append("//")
    ...
    out.append("    // Templated key so callers can pass char32_t (engine input identity) into")
    out.append("    // uint16_t tables without an implicit narrowing conversion (MSVC C4244")
    out.append("    // under /W4 /WX). Keys are BMP letters/digits — the cast is exact.")
    out.append("    template <class Key>")
    out.append("    [[nodiscard]] constexpr const_iterator find(Key k) const noexcept {")
    out.append("        const K key = static_cast<K>(k);")
    ...
    out.append("            if (e[mid].key < key) { lo = mid + 1; } else { hi = mid; }")
    out.append("        }")
    out.append("        if (lo < N && e[lo].key == key) { return const_iterator(e + lo); }")
