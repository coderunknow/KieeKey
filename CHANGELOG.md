...
**Layout (WS-A).** The scroll model is now one unit everywhere (`pagePx = max(1, viewport)`, `rangeMaxPx = contentPx − 1`, `enabled = maxTravelPx > 0`), so content below a tab's viewport is reachable again instead of merely painted. Runtime text can no longer be silently clipped: one-line value rows use `SS_ENDELLIPSIS` + a tooltip (`markOneLineRow`), while rows whose text grows (engine status, arcade status, AI stats, coach advice) grow their box from a real `DrawTextW(DT_CALCRECT)` measurement (`refreshGrowingRow`, grow-only and capped). Every `CBS_DROPDOWNLIST` is created at its **closed** height and asks for its drop-down height afterwards (`applyComboDropHeight` → `CB_SETMINVISIBLE`), removing the 120–200 px invisible window that covered lower-z-order siblings. The Chaos Lab repositions its children from the authored 96-DPI table and re-fits the window on `WM_DPICHANGED` (font + rects + window now scale together). The Arcade Hub footer hint and the FPS counter share `ok::arcadechrome::planFooter()`, so the hint can never run under the counter and the counter follows the DPI. The worst-case note rows (tab 0 `IDC_STAT_OUT_NOTE` et al.) are pinned by name at 100/125/150 %.

**Chẩn đoán (WS-B).** The tab can finally show its own report: a read-only scrollable `EDIT` pane fed by the **same builder the export writes** (pane bytes == exported bytes), "Xem báo cáo" / "Mở file" (`ShellExecuteW`, rc > 32), and a Vietnamese item list for the quick check instead of the raw `engine|backspace|…` token. New `src/app/DiagReportText.hpp` + `tests/test_diag_report_text.cpp`.

**Windows UI probe (WS-C).** New opt-in CMake target `kieekey_ui_probe` (+ `tools/ui_probe/`): it opens the REAL settings dialog through the app's own creation path (`-DKIEEKEY_UI_PROBE` entry points) and checks all nine tabs with real font metrics — no overlap, nothing outside the tab page, every label fits, the scrollbar is enabled exactly when content overflows, every interactive control hit-tests to itself, every tab header fits — writing `ui_probe.json` and one PNG per tab. Wired into the x64 `windows-2022` job; the local audits gained four checks (multi-scale text fit, combo drop-down windows over lower-z-order siblings, group containment, one-line row policy) plus the BS-05 combo contract and the BS-08 note pins, all seed-verified by `tests/verify_audit_seeds.py`.

**v1.3.0 features (WS-D).** **FT-01 (HIGH, silent data loss):** XP, level, achievements and the learned AI profile now persist in `%APPDATA%\KieeKey\progression.dat` and `aiprofile.dat` — saved on both exit paths and every 30 s from the UI tick, loaded fail-open at boot (a corrupt file is reported once and the session starts fresh), AI profile written only while opted in. The engines could always save; nothing under `src/app/` had ever called them. **FT-02:** "Gõ chữ Flexing ra app" reports *why* nothing was typed (`ok::flexsend::Outcome`, a Lab status row, one dialog per session for a refused activation); fail-closed behaviour unchanged. **FT-03/FT-05:** the Live-Effects gate row is on the fit path, the "+1,8 ns/ký tự" note is pinned to `docs/PERFORMANCE.md`, and F9 respects a running arcade game through an atomic mirror published by the UI tick (the hook thread may not call the arcade singletons — enforced by `scripts/check_input_isolation.py`). **FT-04:** the web lab's "N" input is proven to drive `FlexingGame::setGranularity` and its bounds are pinned to the engine clamp.

**Gates.** New/extended: `scripts/audit_feature_persistence.py` (RED with 11 findings on the pre-fix tree), `scripts/audit_live_effects_truth.py` (4 RED), `scripts/audit_chaos_lab.py` (DPI + FT-02 layers), `scripts/audit_layout.py` (+`combo_window`, `pinned_row`), `tests/verify_audit_seeds.py` (9 seeds). New portable suites: `test_arcade_chrome_layout` (177 checks), `test_diag_report_text`, `test_progression_persist` (29 checks), `test_flex_send_outcome` (94 checks); `test_dialog_layout` extended to 14. Local gate `tests/run_all_tests.sh --quick --jobs=2` is ALL GREEN. No engine/hook/TSF pipeline behaviour changed.
