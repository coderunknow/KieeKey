...
#include "ConsonantWalks.hpp"   // v1.3.0 RC1 C-A — the spelling walks as small automata
...
void TextEngine::checkSpelling(bool forceCheckVowel) {
    ...
    if (spellingEndIndex_ > 0) {
        ...
        if (isConsonantAt(0)) {
            ...
            j = walks::matchLeadingConsonant(typingWord_.data(), spellingEndIndex_, maskA, maskB);
        }
        ...
        if (k > j) {   // has vowel
            ...
            if (k >= spellingEndIndex_) {
                ...
                // v1.3.0 RC1 C-A — the RC3 first-letter bucket plus the
                // per-cell walk is now walks::endConsonantMatch (bitmask AND
                // over the 11 rows); the only state the old loop left behind
                // that anything reads is spellingOK_, set on accept only, and
                // the option-mask fold, which moved inside.
                if (walks::endConsonantMatch(tail, tailLen, opts_.quickEndConsonant)) {
                    spellingOK_ = true;
                }
            }
