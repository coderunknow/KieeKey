...
public:
    ...
    bool waitDrained(const std::atomic<std::uint32_t>& pending,
        ...
        const std::uint32_t deadline = tickMs() + kWaitBudgetMs;
        while (true) {
            if (pending.load(std::memory_order_acquire) == 0) { return true; }
...
private:
    ...
    bool waitDrainedImpl(const std::atomic<std::uint32_t>& pending,
                         std::uint32_t spinIters, PauseFn pause) noexcept {
        if (pending.load(std::memory_order_acquire) == 0) { return true; }
        for (std::uint32_t i = 0; i < spinIters; ++i) {
            pause();
            if (pending.load(std::memory_order_acquire) == 0) { return true; }
        ...
        const auto deadline = std::chrono::steady_clock::now() +
                              std::chrono::milliseconds(kWaitBudgetMs);
        while (true) {
            if (pending.load(std::memory_order_acquire) == 0) { return true; }
    ...
    bool waitDrained(const std::atomic<std::uint32_t>& pending,
                     std::uint32_t spinIters = kSpinIters) noexcept {
        return waitDrainedImpl(pending, spinIters, [] { });
    }
