...
PD onHookEvent(const KeyEvent& ev) noexcept {
    ...
    if (reissueTyped) {
        ...
        g.repScratch.push_back(in.kind == InputKind::Space ? L' ' : in.ch);
    }
...
void addTrayIcon() noexcept {
    ...
    if (!g.balloonShown.exchange(true)) {
        ...
        std::wstring title = L"KieeKey v1.0";
...
void showTrayMenu() noexcept {
    ...
    ::AppendMenuW(menu, MF_STRING | (g.engineEnabled.load() ? MF_CHECKED : 0),
                  IDM_TOGGLE, L"Bật gõ tiếng Việt");
    ::AppendMenuW(menu, MF_SEPARATOR, 0, nullptr);
    ::AppendMenuW(methodMenu, MF_STRING | (g.options.inputMethod == InputMethod::Telex ? MF_CHECKED : 0),
                  IDM_METHOD_TELEX, L"Telex");
    ::AppendMenuW(methodMenu, MF_STRING | (g.options.inputMethod == InputMethod::Vni ? MF_CHECKED : 0),
                  IDM_METHOD_VNI, L"VNI");
    ::AppendMenuW(methodMenu, MF_STRING | (g.options.inputMethod == InputMethod::SimpleTelex ? MF_CHECKED : 0),
    ...
    switch (msg) {
        case WM_CREATE: {
            ...
            for (int i = 0; i < 6; ++i) {
                mkCtl(hwnd, L"BUTTON", kOpts[i], WS_CHILD | WS_VISIBLE | BS_AUTOCHECKBOX,
                      28, 118 + i * 26, 320, 20, reinterpret_cast<HMENU>(kIds[i]));
            }
        ...
        case WM_TIMER: {
            ...
            if (s_lastMs != 0 && nowMs > s_lastMs) {
                const double dt  = (nowMs - s_lastMs) / 1000.0;
                const double kpm = ((keys - s_lastKeys) / dt) * 60.0;   // keys/min
                s_kpmEma = (s_kpmEma == 0)
                    ? static_cast<std::int64_t>(kpm)
                    : static_cast<std::int64_t>(s_kpmEma * 0.7 + kpm * 0.3);
            }
            s_lastKeys = keys;
            s_lastMs   = nowMs;
...
int WINAPI wWinMain(HINSTANCE hInst, HINSTANCE, PWSTR, int) {
    // Single instance
    HANDLE mutex = ::CreateMutexW(nullptr, TRUE, L"KieeKey_1.0_Singleton");
