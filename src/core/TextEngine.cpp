...
bool TextEngine::resumeFromText(const std::wstring& rawWord) noexcept {
    ...
    // Quietly replay the letters through the normal per-key state machine so
    // the buffer/typing-word/history state is EXACTLY what it would be if the
    // user had typed them. The per-key outputs are discarded — the text is
    // already on screen; only the state advance is wanted. This is safe
    // because the engine buffer holds raw letters (transforms are emitted as
    // replacement text, never stored), so after the replay the engine's raw
    // buffer matches the visible word and the next key composes onto it.
    for (const wchar_t wc : rawWord) {
        const char32_t c = static_cast<char32_t>(wc);
        TextInput in;
