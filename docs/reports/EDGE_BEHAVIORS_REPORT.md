...
event with the symbol character and the shift flag — not an artificial `WordBreak` event.
  * **NextGen TextEngine** (shipped implementation)
...
**Verdict classes:** PASS · **ENGINE-DEFECT** (NextGen loses data, 2.0.5 correct) ·
205-QUIRK (2.0.5 loses data, NextGen correct) · BOTH-DIFF.
...
NextGen defect shows as engine==oracle != 2.0.5).
...
- `www.facebook.com` -> `ww.facebook.com`: OpenKey (2.0.5 and NextGen) drops the second `w`
...
### 205-QUIRK cases (NextGen correct, legacy 2.0.5 loses data)
...
`Windows?` -> `Windows`), and mishandles `{`/parens; NextGen (matching the oracle) keeps
