...
//============================================================================
//----------------------------------------------------------------------------
// KieeKey v1.1.0 — tests/vi_oracle.hpp
...
private:
    ...
    void mainKeyBranch(char32_t c, bool caps) {
        ...
        // Spec fix (mirrors the engine): a letter arriving while the oracle
        // holds no pending word starts a NEW word — re-arm the macro flag so
        // (a) an identical abbreviation can expand again after a BREAK-KEY
        // (comma/enter) expansion and (b) the spelling-restore path is no
        // longer silently gated for the word following such an expansion
        // (the legacy _hasHandledMacro leak: "uwowng,duwsngkhos " kept
    ...
    void wordBreakBranch(char32_t c, bool caps, const Event& in, bool macroBreak) {
        ...
        if (res_.code == Code::DoNothing) {
            ...
            index_ = 0;
            // Spec fix (mirrors the engine's v1.1.0 break-key ReplaceMacro
            // tail): a break-key expansion replaces the raw keys on screen
            // and consumes the break key, so the next word must start from
            // a clean session — reset the full word state, not just index_
            // (the legacy tail left stale stateIndex_/specialChar_ so the
            // next word composed onto phantom letters).
            if (res_.code == Code::ReplaceMacro) {
                stateIndex_ = 0;
                tempDisableKey_ = false;
                specialChar_.clear();
