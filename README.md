...
* **RC polish (included in this beta7 RC)**: verdict requires evidence (`CHƯA ĐỦ DỮ LIỆU` when no keys observed — no fake OK), unavailable vs zero distinguished with `(chưa có — snapshot chưa làm tươi)` placeholders and `Level::Off` annotated, atomic `codeTableCache` mirror makes hook/consumer/F9/tray share one live-gate model (`liveGateNow()`), and `GetVersionExW` deprecated fallback removed to satisfy MSVC `/WX` (RtlGetVersion only, guard keeps last good value).
